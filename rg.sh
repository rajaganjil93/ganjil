...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://sc1.bagusvpn.me/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://sc1.bagusvpn.me/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://sc1.bagusvpn.me/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://sc1.bagusvpn.me/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://sc1.bagusvpn.me/ip | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>⚡AUTOSCRIPT PREMIUM V16 LTS⚡</b>
<code>────────────────────</code>
...
function menu(){
    ...
    wget --no-check-certificate https://bagusvpn.me/kikitolan/mukung/menu.zip
    unzip menu.zip
    chmod +x menu/*
